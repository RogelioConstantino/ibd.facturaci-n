...
{
    public class ArchivoSegregacionDa
    {
        private readonly IBaseDatos _baseDatos;

        public ArchivoSegregacionDa(IBaseDatos baseDatos)
        {
        ...
        public Int64 actulaizaRegistroHeader(ArchivoSegregacionEn en)
        {
            try
            {
                ConnectionDB con = new ConnectionDB();
                string query = string.Format("[IBD.Facturacion]..[usp_actualizaArchivosSegregacion]");
